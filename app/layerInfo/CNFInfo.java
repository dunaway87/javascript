...
public class CNFInfo {
	...
	ResultSet results = conn.prepareStatement("Select \"NAME\" from cnf where ST_CONTAINS(ST_Transform(my_geom, 4326),ST_SETSRID(ST_POINT("+lng+", "+lat+"), 4326))  limit 1").executeQuery();
	try{
		Logger.info("Select \"NAME\" from cnf where ST_CONTAINS(ST_Transform(my_geom, 4326),ST_SETSRID(ST_POINT("+lng+", "+lat+"), 4326))  limit 1");
		results.next();
